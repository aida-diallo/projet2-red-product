...
//             throw new Error(`HTTP error ${response.status}: ${response.statusText} - ${JSON.stringify(errorData)}`); // More descriptive error
//         }

//         const hotels = await response.json();
//         // ... process the hotels data ...
//     } catch (error) {
//         console.error("Error fetching hotels:", error); // Improved error logging
//     }
// }
...
const ListePage = () => {
  ...
  const fetchHotels = async () => {
    try {
      ...
      setHotels(data.hotels); // Assuming your API returns an array of hotels
    } catch (error) {
  ...
  const addHotel = async (data: any) => {
    try {
      ...
      const newHotel = await response.json();
      setHotels((prev) => [...prev, newHotel.hotel]); // Update hotels state with new hotel
    } catch (error) {
    ...
    <Liste>
      <Container>
        <Row>
          ...
          <Box>
            <Button onClick={() => setModalOpen(true)}>Créer un nouvel hôtel</Button>
